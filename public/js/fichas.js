...
async function cargarFichas() {
    ...
    try {
        ...
        tbody.innerHTML = '<tr><td colspan="7">Error al cargar fichas</td></tr>';
    }
}

async function buscarFicha() {
    const filtro = document.getElementById("txtbuscar").value.toLowerCase().trim();
    if (!filtro) return cargarFichas();
    
    try {
        const res = await fetch(`http://localhost:5000/fichas?search=${filtro}`);
        const fichas = await res.json();
        mostrarResultadosFichas(fichas);
    } catch (err) {
        console.error("Error al buscar fichas:", err);
    }
}

function mostrarResultadosFichas(fichas) {
    const tbody = document.getElementById("tabla-fichas-body");
    tbody.innerHTML = "";
...
function verFicha(id) {
    console.log('Navegando a ver ficha con ID:', id);
...
async function eliminarConfirmado() {
    try {
        const res = await fetch(`http://localhost:5000/fichas/${idAEliminar}`, {
            method: 'DELETE'
        });
        if (res.ok) {
            cargarFichas();
